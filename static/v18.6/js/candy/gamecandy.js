 ...
 window.Candy.Game = function() {};
 window.Candy.Game.prototype = {
     game: null,
     stage: null,
     renderer: null,
     column1: {},
     ...
     initGame: function() {
         ...
         $(".minicandy_game_v2").append(this.game.view);
     ...
     getItem: function(id, isblur) {
         ...
         if (isblur) {
             return 'mini_candy_item_' + id + '_blur' + '.png';
         } else {
             if (id === 0 || id === '0') {
                 return 'mini_candy_item_bg_item.png';
             } else {

                 return 'mini_candy_item_' + id + '.png';
             }
         }
     },
     ...
     initAsset: function() {
         ...
         $('#Candy .mini_close').unbind('click').click(function() {
             commonCandy.hideGUI();
         });


         $('#Candy .mini_help').unbind('click').click(function() {
             commonCandy.showGuide();
         });


         $('#Candy .mini_rank').unbind('click').click(function() {
             commonCandy.showRank();
         });


         $('#Candy .mini_history').unbind('click').click(function() {
             commonCandy.showHistory();
         });
         ...
         $('#Candy .room100').unbind('click').click(function() {
             if (commonCandy.playing)
                 return;
             $('#candy_room div').removeClass('active');
             $('#Candy .room100').addClass('active');
         ...
         $('#Candy .room1K').unbind('click').click(function() {
             if (commonCandy.playing)
                 return;
             $('#candy_room div').removeClass('active');
             $('#Candy .room1K').addClass('active');
             commonCandy.ChangeBetValue(1000, 2);
         });

         $('#Candy .room10K').unbind('click').click(function() {
             if (commonCandy.playing)
                 return;
             $('#candy_room div').removeClass('active');
             $('#Candy .room10K').addClass('active');
             commonCandy.ChangeBetValue(10000, 3);
         });


         $('#Candy .room5K').unbind('click').click(function() {
             if (commonCandy.playing)
                 return;
             $('#candy_room div').removeClass('active');
             $('#Candy .room5K').addClass('active');
             commonCandy.ChangeBetValue(5000, 4);
         });
         ...
         $('#Candy .btn_auto').unbind('click').click(function() {
             if (commonCandy.isAuto) {
                 commonCandy.isAuto = false;
                 $('#Candy .btn_auto').removeClass('active');
             } else {
                 $('#Candy .btn_auto').addClass('active');
                 commonCandy.isAuto = true;
         ...
         $('#Candy .btn_sieutoc').unbind('click').click(function() {
             that.mouseDonwSpinSpeed();
         });



         $('#Candy .btn_spin').unbind('click').click(function() {
             that.mouseDonwSpin();
         });

         $('#Candy .btn_line').unbind('click').click(function() {
     ...
     mouseDonwSpin: function() {
         var that = commonCandy.GameCandy;
         if (commonCandy.playing)
             return;
         that.disableButton();
         commonCandy.spins();
     ...
     mouseDonwSpinSpeed: function() {
         ...
         if (that.timeSpeed === 1) {
             $('#Candy .btn_sieutoc').addClass('active');
             that.timeSpeed = 0.5;
             if (commonCandy.playing)
                 return;
             commonCandy.spins();
             ...
             $('#Candy .btn_sieutoc').removeClass('active');
             that.timeSpeed = 1;
         }
     ...
     beforeSpin: function() {
         var that = this;
         var objeffect = that.mainContainer.getChildByName('effect_candy');
         if (objeffect != null) {
             objeffect.destroy();
         }
         that.resetLines();
         clearTimeout(commonCandy.timeOutEachLine);
         clearTimeout(commonCandy.timeIntEachLine);
         clearTimeout(commonCandy.timeSpin);
     },
     ...
     startSpin: function(result) {
         ...
         if (listPrizeData.length > 0) {
             for (var i = 0; i < listPrizeData.length; i++) {
                 commonCandy.listLine.push(parseInt(listPrizeData[i].split(",")[0]));
             }
         }
     ...
     effect: function(id, result) {
         ...
         containerEffect.name = 'effect_candy';
         if (id === 1) {
             ...
             TweenMax.to(txtJack, 2, {
                 ...
                 onComplete: function() {
                     commonGame.showVqJackpot(that, 225, 'Candy', result.JackpotPrizeCurr, result.JackpotRateCurr, result.JackpotPrizeRateCurr);
                 }
             });
     ...
     updateTextJack: function(jack) {
         var that = this;
         var obj1 = that.mainContainer.getChildByName('effect_candy');
         if (obj1 != null) {
             var obj = obj1.getChildByName('txtJackCandy');
             if (obj != null) {
     ...
     normalWinOrJackPotOrBigWin: function(result) {
         ...
         if (result.TotalPrizeValue > 0) {
             ...
             that.resetLines();
             for (var i = 0; i < commonCandy.listLine.length; i++) {
                 that.showLineId(commonCandy.listLine[i]);
             }
             if (result.IsJackpot) {
                 ...
                 commonCandy.timeSpin = setTimeout(function() {
                     if (commonCandy.isAuto) {
                         if (commonCandy.playing)
                             return;
                         that.disableButton();
                 ...
                 commonCandy.timeOutEachLine = setTimeout(that.showEachLine, 1000);
             } else {
     ...
     showEachLine: function() {
         var that = commonCandy.GameCandy;
         clearTimeout(commonCandy.timeOutEachLine);
         var lineCount = commonCandy.listLine.length;
         var i = 0;
         if (lineCount === 0) {
             clearTimeout(commonCandy.timeIntEachLine);
             return;
         }
         commonCandy.timeIntEachLine = setInterval(function() {
             if (commonCandy.playing) {
                 clearTimeout(commonCandy.timeIntEachLine);
                 return;
             }
     ...
     disableButton: function() {
         var that = this;
         $('#Candy .btn_spin').addClass('disable');

     },
     enableButton: function() {
         var that = this;
         $('#Candy .btn_spin').removeClass('disable');
         $('#Candy .btn_sieutoc').removeClass('active');
         that.timeSpeed = 1;
     },
     resetLines: function() {
     ...
     btnTintOver: function() {
         this.filters = [commonCandy.GameCandy.filter];
     },
     btnTintOut: function() {
         this.filters = null;
     },
     ...
     deleteTweens: function() {
         ...
         for (var i = 0; i < all.length; i++) {
             if (all[i].data === "candy-effect" || all[i].data === "candy-scale" || all[i].data === "candy-move")
                 all[i].kill();
         }
     },
 };